...
function check_down(grid) {
  ...
  for (var i = 0; i < 17; i++) {
    for (var j = 0; j < 20; j++) {
      ...
      if (prod > d_answer) { d_answer = prod }
    }
  }
  return d_answer
}

function check_right(grid) {
  var prod, r_answer = 0

  for (var i = 0; i < 20; i++) {
    for (var j = 0; j < 17; j++) {
      prod = grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3]
      if (prod > r_answer) { r_answer = prod }
    }
  }
  return r_answer
}
